...
#install.packages(c("Matrix","data.table","plyr","reshape","ggplot2"))
...
head(lfdat)
```
Compile objective files that are needed.


```{r,eval=F}
compile(paste(dllroot,"tmb/hier_cL_CSD.cpp",sep=""))
compile(paste(dllroot,"tmb/hier_cL_LSD.cpp",sep=""))
compile(paste(dllroot,"tmb/hier_cL_CSD_OBSLL.cpp",sep=""))
compile(paste(dllroot,"tmb/hier_cL_LSD_OBSLL.cpp",sep=""))

compile(paste(dllroot,"tmb/hier_ck_CSD.cpp",sep=""))
compile(paste(dllroot,"tmb/hier_ck_LSD.cpp",sep=""))
compile(paste(dllroot,"tmb/hier_ck_CSD_OBSLL.cpp",sep=""))
compile(paste(dllroot,"tmb/hier_ck_LSD_OBSLL.cpp",sep=""))

compile(paste(dllroot,"hier_yk_CSD.cpp"))
compile(paste(dllroot,"hier_yk_LSD.cpp"))
compile(paste(dllroot,"hier_yk_CSD_OBSLL.cpp"))
compile(paste(dllroot,"hier_yk_LSD_OBSLL.cpp"))
``` 
...
source(paste(dllroot,"R/hier_fun.R",sep=""))
```
...
niter=10000,rel.tolerance=1e-8,dllroot = dllroot)
...
niter=10000,rel.tolerance=1e-8,dllroot = dllroot,sdl=-2,sdL=-2)
```

Load the test object if you haven't run the model
```{r}
load(paste(dllroot,"data/hier_test.RData",sep=""))
```
\newpage
#Results
...
```{r}
source(paste(dllroot,"R/plot_lfem.R",sep=""))
library(grid)
library(gridBase)
plot.lfem(model=hier_test,Lengths=lfdat,Survey.num=2,xlimit=140,td=T)
```
