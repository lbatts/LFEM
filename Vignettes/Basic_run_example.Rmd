...
```
The dataframe should be formatted as below for the function, with four columns: Survey, Year, Length and RF (raising factor).  The data has been aggreagted by year, survey and length so as to compact the dataframe for speed. Observed log-likelihood is the same if individual fish lengths for each haul were used.
...
for(i in 1:dim(df)[1]){
  ...
  result_TMB<-basic(year0=year0,no.years=no.years, age0=age0, L=df[i,2], l=16, k.reparam=df[i,1], sigma.start=c(10),No.comp=df[i,3],
                                                       Lengths=lfdata2,niter=10000,SD.type=4,rel.tolerance=1e-8)
