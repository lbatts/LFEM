hier_constantSD_yk<-function(year0,no.years, age1, L, l, k.reparam, sigma.start,No.comp, Lengths,niter,rel.tolerance)
...
#####set up Mu's. If no variation between years than only need one set one Mus
 ...
 dyn.load(dynlib(paste0(dllroot,"hier_model_version_constantSD_plusyearlyk_adj")))
 dyn.load(dynlib(paste0(dllroot,"hier_model_version_constantSD_plusyearlyk_OBSLL_adj")))
...
for(k in 1:niter){ 
  ...
  obj_LL <- MakeADFun(
    ...
    DLL = "hier_model_version_constantSD_plusyearlyk_OBSLL_adj",silent=T)
    ...
    if(k > 2 ){
        if(abs(obs.llike[k] - obs.llike[k-1]) <  abs(obs.llike[k-1] * rel.tolerance)){
          ...
          dyn.unload(dynlib(paste0(dllroot,"hier_model_version_constantSD_plusyearlyk_adj")))
          dyn.unload(dynlib(paste0(dllroot,"hier_model_version_constantSD_plusyearlyk_OBSLL_adj")))
          ...
          linf_cohort<-(mu.arr.bff[No.comp:max.years.comp,No.comp,1]-(mu.arr.bff[1:max.years.backforfill,1,1]*k.reparam.mean.em^(No.comp-1)))/(1-(k.reparam.mean.em^(No.comp-1)))
          tzero_cohort<-  age1[1] -((1/log(k.reparam.mean.em))*log((mu.arr.bff[No.comp:max.years.comp,No.comp,1]-mu.arr.bff[1:max.years.backforfill,1,1])/(mu.arr.bff[No.comp:max.years.comp,No.comp,1]-(mu.arr.bff[1:max.years.backforfill,1,1]*k.reparam.mean.em^(No.comp-1)))))
          
            K_yearly<- -log(k.par.vec.plus)
            ...
            K.yearly = K_yearly,Linf.cohort=linf_cohort,tzero.cohort = tzero_cohort, Lambda.params=no.lambda.param,Rho=rho.em,
...
obj <- MakeADFun(
  ...
  DLL = "hier_model_version_constantSD_plusyearlyk_adj",silent=T)
...
tzero.em<-  age1[1]-((1/log(cohort.k.rep.means))*log((L-l.par.vec.plus)/(L-(l.par.vec.plus*cohort.k.rep.means^((No.comp-1)))))) 
