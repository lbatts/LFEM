...
l.par.vec.plus  <- rep(l,max.years.backforfill)
 ...
 dyn.load(dynlib(paste0(dllroot,"hier_model_version_constantSD_pluscohortk_adj")))
 dyn.load(dynlib(paste0(dllroot,"hier_model_version_constantSD_pluscohortk_OBSLL_adj")))
...
for(k in 1:niter){ 
  ...
  obj_LL <- MakeADFun(
    ...
    DLL = "hier_model_version_constantSD_pluscohortk_OBSLL_adj",silent=T)
...
obj <- MakeADFun(
  ...
  DLL = "hier_model_version_constantSD_pluscohortk_adj",silent=T)
