...
Type objective_function<Type>::operator() () {
  ...
  PARAMETER(log_comp_sigma); // consatnt sd 
  ...
  Type comp_sigma= exp(log_comp_sigma);
  ...
  vector <Type> k_par_vec_plus(No_maxyearbff);
  ...
  if(no_surveys >1){
    for(int j = 1; j<no_surveys;j++){
      for(int i = 0; i < No_maxyearbff;i++){
        K = -log(k_par_vec_plus[i]);
        linf = (mu(i+No_comp-1,No_comp-1,0)-(mu(i,0,0)*(pow(k_par_vec_plus[i],No_comp-1))))/(1-(pow(k_par_vec_plus[i],No_comp-1)));
        tzero =  agezero[0]-((1/log(k_par_vec_plus[i]))*log((mu(i+No_comp-1,No_comp-1,0)-mu(i,0,0))/(mu(i+No_comp-1,No_comp-1,0)-(mu(i,0,0)*(pow(k_par_vec_plus[i],No_comp-1))))));
    ...
    //std::cout << "mu" << std::endl << mu.col(0).matrix() << std::endl << std::endl;
    ...
    for(int i=1;i<No_mycomp;i++){
      ...
      for(int w=1;w<(No_comp-1);w++){
        ...
        if(diffiw >= 0 && diffiw < No_maxyearbff){
          ...
          k_reparam = k_par_vec_plus[diffiw];
        }
        else {
  ...
  for(int i = 0; i < n; i++){ 
    ...
    for(int j = 0; j < t; j++){
      ...
      tmp[j] = ((lam3[j])*(dnorm(x[i], new_mu2(year,j), comp_sigma)));
